...
class HierarchicalCommunitySearcher:
    ...
    def hierarchical_community_search(self, verbosity: int = 0, max_depth: int | None = None, division_tree: bool = False) -> list:
        if verbosity >= 1:
            print("Starting community detection")
        ...
        if max_depth == None:
            ...
            result = self._hierarchical_search_recursion(verbosity=verbosity, level=1, max_depth=max_depth, division_tree=division_tree)
            ...
            if division_tree:
                ...
                higher_list_elements = self._flatten_list_to_set(division_tree[len(division_tree) - 2])
                lower_list_elements = self._flatten_list_to_set(division_tree[len(division_tree) - 1])
    ...
    def _hierarchical_search_recursion(self, verbosity: bool, max_depth: int, level: int, community: list | None = None, division_tree: list | None = None):
        if not community:
            community = [*range(self.sampler.G.number_of_nodes())]
        ...
        if level == max_depth:
            ...
            if c0 and c1:
                return (self._hierarchical_search_recursion(verbosity, max_depth, level=level+1, community=c0, division_tree=division_tree) + 
                        self._hierarchical_search_recursion(verbosity, max_depth, level=level+1, community=c1, division_tree=division_tree))
            elif c0:
                return [c0]
            else:
                return [c1]
