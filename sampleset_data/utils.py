        ...
        if level == len(division_tree) - 1:
            ...
            for subsequent_cluster in subsequent_clusters:  # children clusters
                for cluster in level_clustering:  # parent clusters
                    if set(subsequent_cluster).issubset(
                        set(cluster)
                    ):  # if subsequent cluster is child of the parent
...
def build_graph(G, node, division_modularities, level=0):
    ...
    color = (
        "#4c72b0" if node.left or node.right else "#d35d6e"
    )  # blue for internal, red for leaves
    values = node.values
...
def center_parents(G, pos):
    ...
    for n, data in sorted(
        G.nodes(data=True), key=lambda x: x[1]["level"], reverse=True
    ):
...
class DivisionNode:
    def __init__(
        self,
        community_hash: str | int,
        node: Node,
        sampleset_data: SamplesetData | None,
    ):
...
#         nodes_k: (
#             DivisionNode(
#                 community_hash=node.id,
#                 node=node,
#                 sampleset_data=sampleset_metadata[nodes_k],
#             )
#             if nodes_k in sampleset_metadata.community_hash
#             else DivisionNode(community_hash=node.id, node=node, sampleset_data=None)
#         )
#         for nodes_k, node in nodes.items()
#     }
...
#     root_extended = DivisionNode(
#         community_hash=root.id, node=root, sampleset_data=sampleset_metadata[root.id]
#     )
#     return extended_nodes, root_extended

def recover_info_ordering(root, nodes, sampleset_metadata):
    extended_nodes = {
        nodes_k: DivisionNode(
            community_hash=node.id,
            node=node,
            sampleset_data=sampleset_metadata.get_with_hash_id(nodes_k)
            if nodes_k in sampleset_metadata.community_hash
            else None
        )
        for nodes_k, node in nodes.items()
    ...
    root_extended = DivisionNode(
        community_hash=root.id,
        node=root,
        sampleset_data=sampleset_metadata.get_with_hash_id(root.id)
        if root.id in sampleset_metadata.community_hash
        else None
    )
...
def plot_tree_extended(
    root, division_modularities, figsize=(14, 8), cmap=plt.cm.viridis, value=None
):
    ...
    cbf_occured = [
        n
        for n in G.nodes()
        if G.nodes[n]["chain_break_fraction"] is not None
        and G.nodes[n]["chain_break_fraction"] > 0.0
    ]
    ...
    nx.draw_networkx_edges(
        G, pos, arrows=False, edge_color="#7d8ca3", width=1.0, ax=ax, alpha=0.6
    )
    ...
    # none_color = "#CE6060"
    colors = {
        v: mcolors.to_hex(cmap(norm(v))) if v is not None else none_color
        for v in values
    }
    ...
    for node, (x, y) in pos.items():
        ...
        ch_str_label = (
            f"{chain_strength:.3f}" if chain_strength is not None else chain_strength
        )
        label2 = f"Ch.str.: {ch_str_label}"
        cbf_label = (
            f"{chain_break_fraction:.3f}"
            if chain_break_fraction is not None
            else chain_break_fraction
        )
    ...
    for lvl, m in zip(levels, modularities):
        ...
        if nodes_at_level:
            ...
            plt.text(
                xmax + dx / 2,
                np.mean([pos[n][1] for n in nodes_at_level]),
                f"{m:.4f}",
                ha="left",
                va="center",
                fontsize=9,
            ...
            plt.hlines(
                y=np.mean([pos[n][1] for n in nodes_at_level]),
                xmin=(xmax + dx) - (xmax + dx),
                xmax=xmax + dx / 2,
                colors="#bbbbbb",
                linestyles="dashed",
                lw=0.8,
    ...
    plt.title(
        "Chain strength & problem size in the hierarchical division tree",
        fontsize=16,
        fontweight="bold",
        color="#2b3a42",
        pad=20,
