...
class LeidenSampler(RegularSampler):
    ...
    def sample_qubo_to_dict(self) -> dict: 
        communities = list(la.find_partition(ig.Graph.from_networkx(self.G), partition_type=la.ModularityVertexPartition))
        self.communities_number = len(communities)
        sample = communities_to_dict(communities)
        return sample
    
    def sample_qubo_to_list(self) -> list: 
        sample = list(la.find_partition(ig.Graph.from_networkx(self.G), partition_type=la.ModularityVertexPartition))
        return sample